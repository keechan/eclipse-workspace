...
%>
<script>  
  function delK(start, ch1, ch2, idx){
	//  alert(start+ ch1+ ch2 +idx )
	location.href="<%=path %>/PageBoardController?sw=D&start="+start+"&ch1="+ch1+"&ch2="+ch2+"&idx="+idx
  }
...
<section>
 <br>
   ...
   <tr>
     <th>rownum </th><th>rnum </th><th>번호 </th><th>이름 </th>
     <th>제목 </th><th>조회수</th> <th>삭제</th>     
   </tr>
   <%
   if (ch2 != null ){
   ...
   <tr align="center">
      ...
      <td><%=m.getCnt() %></td> 
       <td><input type=button  value="삭제" onClick="delK('<%=start %>','<%=ch1 %>','<%=ch2 %>','<%=m.getIdx()%>')"></td>      
